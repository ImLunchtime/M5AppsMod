...
#endif
{
    ...
    if (espnow_data->version != ESPNOW_VERSION || (espnow_data->type >= ESPNOW_DATA_TYPE_MAX) || size != espnow_data->size + sizeof(espnow_data_t) || ESPNOW_ADDR_IS_SELF(espnow_data->src_addr))
    {
    ...
    if (!g_recv_handle[espnow_data->type].enable && (!g_espnow_config->forward_enable || !frame_head->broadcast || !frame_head->forward_ttl))
    {
        return;
    }
    ...
    if (g_recv_handle[espnow_data->type].enable && espnow_data->type != ESPNOW_DATA_TYPE_ACK && espnow_data->type != ESPNOW_DATA_TYPE_GROUP && frame_head->ack && ESPNOW_ADDR_IS_SELF(espnow_data->dest_addr))
    {
    ...
    else
    {
        for (size_t i = 0, index = g_msg_magic_cache_next; i < ESPNOW_MSG_CACHE;
             i++, index = (g_msg_magic_cache_next + i) % ESPNOW_MSG_CACHE)
        {
            if (g_msg_magic_sec_cache[index].type == espnow_data->type && g_msg_magic_sec_cache[index].magic == frame_head->magic)
            {
                return;
            }
        }
    ...
    {
        if (!frame_head->group && frame_head->broadcast && !ESPNOW_ADDR_IS_BROADCAST(espnow_data->dest_addr) && !ESPNOW_ADDR_IS_SELF(espnow_data->dest_addr))
        {
            goto FORWARD_DATA;
        }
...
FORWARD_DATA:
    ...
    if (g_espnow_config->forward_enable && frame_head->forward_ttl > 0 && frame_head->broadcast && frame_head->forward_rssi <= rx_ctrl->rssi && !ESPNOW_ADDR_IS_SELF(espnow_data->dest_addr) && !ESPNOW_ADDR_IS_SELF(espnow_data->src_addr))
    {
                      ...
                      size_t size, const espnow_frame_head_t *data_head, TickType_t wait_ticks)
{
    ...
    if (g_espnow_config->sec_enable && (data_head ? data_head->security : g_espnow_frame_head_default.security) && type != ESPNOW_DATA_TYPE_ACK && type != ESPNOW_DATA_TYPE_FORWARD && type != ESPNOW_DATA_TYPE_SECURITY_STATUS && type != ESPNOW_DATA_TYPE_SECURITY)
    {
    ...
    {
        for (int i = 0; i == 0 || (g_set_channel_flag && frame_head->channel == ESPNOW_CHANNEL_ALL && i < g_self_country.nchan); ++i)
        {
            ...
            do
            {
                ...
                {
                    bool ack = 0;
                    write_ticks = (wait_ticks == portMAX_DELAY) ? portMAX_DELAY : xTaskGetTickCount() - start_ticks < wait_ticks ? wait_ticks - (xTaskGetTickCount() - start_ticks)
                                                                                                                                 : 0;
...
EXIT:
    ...
    if (frame_head->ack && !ESPNOW_ADDR_IS_BROADCAST(dest_addr))
    {
        ...
        while (g_ack_queue && xQueueReceive(g_ack_queue, &ack_magic, MIN(write_ticks, g_espnow_config->send_max_timeout)) == pdPASS)
        {
            if (*ack_magic == frame_head->magic)
            {
                ESP_FREE(espnow_data);
                           ...
                           bool type, TickType_t wait_ticks)
{
    ...
    for (int i = 0; addrs_num > 0; ++i)
    {
        ...
        for (int count = 0; count < frame_head->retransmit_count; ++count)
        {
            for (int i = 0; i == 0 || (frame_head->channel == ESPNOW_CHANNEL_ALL && i < g_self_country.nchan && g_set_channel_flag); ++i)
            {
                ...
                if (ret == ESP_OK)
                {
                    TickType_t write_ticks = (wait_ticks == portMAX_DELAY) ? portMAX_DELAY : xTaskGetTickCount() - start_ticks < wait_ticks ? wait_ticks - (xTaskGetTickCount() - start_ticks)
                                                                                                                                            : 0;
